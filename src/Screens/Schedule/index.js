      ...
      {conflicts.map((conflict, index) => (
        <tr key={index}>
          <td>
            ...
            <p>{conflict[0].day} : {formattedTime(conflict[0].starttime)} - {formattedTime(conflict[0].endtime)}</p>
            {/* Render other properties as needed */}
          </td>
          <td>
            <p>BN#: {conflict[1].building_number} Room: {conflict[1].roomname}</p>
            <p>{conflict[1].day} : {formattedTime(conflict[1].starttime)} - {formattedTime(conflict[1].endtime)}</p>
            {/* Render other properties as needed */}
          </td>
        </tr>
      ))}
